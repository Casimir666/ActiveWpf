...
namespace Lightstreamer.DotNet.Client
{
  public class LSClient
  {
    ...
    public virtual void OpenConnection(ConnectionInfo info, IConnectionListener listener)
    {
      ...
      {
        MyServerListener closeListener = new MyServerListener(this, listener, currPhase);
        bool flag = false;
        try
        {
          ServerManager closingManager = new ServerManager(info1, (ServerManager.IServerListener) closeListener);
          ...
          lock (this.stateMutex)
          {
            if (currPhase == this.phase)
            {
              ...
              this.asyncListener = closeListener;
              this.connManager = closingManager;
            }
            else
            {
              LSClient.CloseFlushing(closingManager, closeListener, listener);
              return;
            }
          }
        ...
        finally
        {
          if (!flag)
          {
            closeListener.OnClosed((IConnectionListener) null);
            closeListener.OnEndMessages();
          }
        }
      }
    }
