...
namespace Lightstreamer.DotNet.Client
{
  internal class HttpProvider
  {
    ...
    protected internal virtual HttpWebRequest SendPost()
    {
      ...
      HttpProvider.MyRequestState myRequestState = new HttpProvider.MyRequestState()
      {
        request = webRequest
      };
      if (webRequest.BeginGetRequestStream(new AsyncCallback(HttpProvider.WriteCallback), (object) myRequestState) == null)
        throw new IOException("Request submission failed unexpectedly (1)");
      if (!myRequestState.allDone.WaitOne(timeoutProp))
      {
        webRequest.Abort();
        throw new IOException("Connection timed out");
      }
      if (myRequestState.ioException != null)
        throw myRequestState.ioException;
      if (myRequestState.webException != null)
        throw myRequestState.webException;
      if (myRequestState.stream == null)
      {
        webRequest.Abort();
        throw new IOException("Request submission failed unexpectedly (2)");
      }
      Stream stream = myRequestState.stream;
    ...
    internal virtual Stream DoHTTP(bool isPost)
    {
      ...
      try
      {
        ...
        if (httpWebRequest.BeginGetResponse(new AsyncCallback(HttpProvider.ReadCallback), (object) myRequestState) == null)
        ...
        webResponse = myRequestState.response;
