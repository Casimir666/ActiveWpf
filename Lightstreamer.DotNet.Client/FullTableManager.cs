...
namespace Lightstreamer.DotNet.Client
{
  internal class FullTableManager : ITableManager
  {
    ...
    {
      FullTableManager.ItemInfo[] itemInfoArray = new FullTableManager.ItemInfo[this.itemInfos.Count];
      lock (this.itemInfos)
      {
        itemInfoArray = (FullTableManager.ItemInfo[]) CollectionsSupport.ToArray((ICollection) this.itemInfos, (object[]) itemInfoArray);
        this.unsubscrDone = true;
      }
      for (int index = 0; index < itemInfoArray.Length; ++index)
      {
        if (itemInfoArray[index] != null)
          this.NotifyUnsubForItem(itemInfoArray[index].pos, itemInfoArray[index].name);
      }
      try
      {
