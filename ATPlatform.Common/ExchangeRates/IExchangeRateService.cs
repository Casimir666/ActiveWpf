...
// Type: ATPlatform.Common.ExchangeRates.IExchangeRateService
...
namespace ATPlatform.Common.ExchangeRates
{
  ...
  {
    void Initialize(IEnumerable<ICurrency> currencies);

    void UpdateExchangeRate(IExchangeRateSymbol symbol, ITick tick);

    ExchangeRate GetRate(string fromCurrencyIsoCode, string toCurrencyIsoCode);
  }
}
