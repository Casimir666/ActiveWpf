...
// Type: ATPlatform.Model.Proto.EntityMappings.AccountEquityMapping
...
{
  internal class AccountEquityMapping : IProtoMapper<AccountEquityProto, AccountEquity>
  {
    public AccountEquity ModelFromProto(AccountEquityProto proto) => new AccountEquity()
    {
      AccountId = (int) proto.AccountId,
      Equity = proto.Equity
    };

    public AccountEquityProto ProtoFromModel(AccountEquity model) => new AccountEquityProto()
    {
      AccountId = (long) model.AccountId,
      Equity = model.Equity
    };
  }
}
